...
class ExpenseController extends BaseController {
  ...
  /**
   * @swagger
   * /pools/{poolId}/expenses/upcoming:
   *   get:
   *     tags: [Expenses]
   *     summary: List upcoming recurring expenses
   *     description: |
   *       Returns paginated recurring expenses whose next occurrence is in the future,
   *       ordered by next_occurrence_at ascending (soonest first).
   *       Only active schedules are included (recurrence_end_date not yet passed).
   *     security:
   *       - bearerAuth: []
   *     parameters:
   *       - in: path
  ...
  async listUpcomingExpenses(req: Request) {
    ...
    return this.expenseService.listUpcomingExpenses(
      req.params['poolId'] as string,
      userId,
      page,
      limit,
